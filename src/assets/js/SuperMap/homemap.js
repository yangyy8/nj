...
export function getSearch(id, mc, num) {
  ...
  var searchResult = [{
    ...
    if (sz < 11) {
      ...
      markerLayer.clearLayers();
      layerGroup.clearLayers();
      var searchResult = window.vm.getpcs(searchResult[0].dm, function(data) {
        if (data != undefined) {
...
//调用数据集获取坐标   分局
 function queryZrqByServer(data) {
...
function renderPCS(data) {
  ...
  var layerss = [];
    ...
    for (var i = 0; i < features.length; i++) {
      ...
      layerss.push(resultLayer);

      var center = L.latLngBounds(changeLonAndLat(features[i].geometry.coordinates)).getCenter();
      //features[i].properties.DWDM;
      renderMarkerpcs([center.lat, center.lng], data);
      layerGroup = L.layerGroup(layerss);
